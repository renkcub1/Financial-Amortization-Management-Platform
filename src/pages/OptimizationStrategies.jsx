...
const OptimizationStrategies=()=> {
  ...
  const strategies=[ 
    {id: 'avalanche',name: 'Debt Avalanche',description: 'Pay minimums on all debts,then put extra money toward the highest interest rate debt',icon: FiTarget,color: 'text-danger-600',bgColor: 'bg-danger-50'},
    {id: 'snowball',name: 'Debt Snowball',description: 'Pay minimums on all debts,then put extra money toward the smallest balance',icon: FiTrendingUp,color: 'text-primary-600',bgColor: 'bg-primary-50'},
    {id: 'hybrid',name: 'Hybrid Strategy',description: 'Balance between psychological wins and mathematical optimization',icon: FiZap,color: 'text-warning-600',bgColor: 'bg-warning-50'} 
  ];
  ...
  const comparisonChartOption={
    tooltip: {
      ...
      formatter: function(params) {
        ...
        })}<br/> Payoff Time: ${params[1].value} months`;
      }
    },
    legend: {
            ...
            > 
              <div className="flex items-start space-x-3">
                <div className={`flex-shrink-0 p-2 rounded-lg ${strategy.bgColor}`}>
                  <SafeIcon
                    icon={strategy.icon}
                    className={`h-5 w-5 ${strategy.color}`}
                  /> 
                </div>
                <div>
            ...
            <Card>
              <div className="flex items-center">
                ...
                <div className="ml-5">
                  ...
                  <p className="text-2xl font-semibold text-gray-900">
                    {selectedStrategy==='avalanche' ? 'Optimal' : selectedStrategy==='snowball' ? 'Motivational' : 'Balanced'} 
                  </p>
                </div>
              </div>
            </Card>
              ...
              {Object.entries(comparisonData).map(([strategyKey,data])=> ( 
                ...
                >
                  <h4 className="font-semibold text-gray-900 capitalize">
                    {strategyKey==='avalanche' ? 'Debt Avalanche' : strategyKey==='snowball' ? 'Debt Snowball' : 'Hybrid Strategy'} 
                  </h4>
